...
if [[ ! -x "$CODEXBAR" ]]; then CODEXBAR="$(command -v codexbar || true)"; fi
CONFIG_PATH="${CODEXBAR_CONFIG:-${XDG_CONFIG_HOME:-$HOME/.config}/codexbar/config.json}"
if [[ -z "${CODEXBAR_CONFIG:-}" && -f "$HOME/.codexbar/config.json" ]]; then CONFIG_PATH="$HOME/.codexbar/config.json"; fi
...
fetch_provider() {
    ...
    if [[ -z "$body" ]]; then
        body="$(jq -cn --arg provider "$p" '[{provider:$provider,error:{message:"Provider returned no data or timed out"}}]')"
    fi
    echo "$body"
}
...
LAST_GOOD="$CACHE_DIR/success.json"
...
# The GUI reads the current display snapshot, including errors and stale flags.
# Successful history remains separate for future offline recovery.
current_tmp="$(mktemp "$CACHE_DIR/.last.XXXXXX")"
printf '%s\n' "$merged" > "$current_tmp"
mv "$current_tmp" "$CACHE_DIR/last.json"
